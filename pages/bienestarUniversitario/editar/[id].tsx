...
function EditarServicioPage() {
  ...
  const [modules, setModules] = useState<IListarServicios[]>([]);
  ...
  const inputFileImgServ: any = useRef(null);
  ...
  async function cargarModulos() {
    try {
      const res = await fetch(`${baseURL}/Servicios/getServicioByModuloId/1`);
      if (!res.ok) {
  ...
  async function cargarDatosModulo(id: number) {
    try {
      const res = await fetch(`${baseURL}/Servicios/getServicioById/${id}`);
      if (!res.ok) {
  ...
  const editarModulo = async (id: number) => {
    ...
    const repetitiveModule = modules.find(
      (m: any) =>
        m.nombre === moduloData.nombre && m.nombre != moduloBkData.nombre
    );
  ...
  const editarUbicacion = async (idMod: number) => {
    ...
    if (send) {
      ...
      console.log(ubicacionData.identificador);
  ...
  const clearImgServ = () => {
    setImg(null);
    if (inputFileImgServ.current) {
      inputFileImgServ.current.value = null;
    }
  };
