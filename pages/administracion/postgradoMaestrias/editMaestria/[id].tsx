...
function EditMaestria({ id }: props) {
  const router = useRouter();
  const [showAlert, setShowAlert] = useState<boolean>(false);
  const [maestria, setMaestria] = useState<IMaestriaData>();
  ...
  useEffect(() => {
    async function doFetch() {
      fetch(`${URL.baseUrl}/api/Facultad/ListaActivos`)
        .then((res) => res.json())
        .then((res) => setMaestria(convertJSONListMaestria(res.response)));
    }
    doFetch();
  }, []);
  ...
  useEffect(() => {
    async function doFetch() {
      fetch(`${URL.baseUrl}/api/Carrera/Obtener/${id}`)
        .then((res) => res.json())
        .then((res) => {
          const maestriaData = convertJSONMaestria(res.response);
          console.log("Datos de la API:", maestriaData); // Agregar este console.log
  ...
  useEffect(() => {
    if (maestria?.titulo) {
      setTitulo(maestria!.titulo);
      setModalidad(maestria!.modalidad);
      setImagen(maestria!.imagen);
    }
  }, [maestria]);
  ...
  return (
    <Layout>
      <PageTitle>Editar Maestria</PageTitle>
      <div className="mb-4">
      ...
      <div className="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
        <form id="miFormulario" onSubmit={handleSubmit}>
          <Label>
            ...
            <Input
              className="mt-1"
              placeholder="Ingresa el titulo de la maestria."
              value={titulo}
          ...
          <Label className="mt-3">
            ...
            <Textarea
              ...
              className="mt-1"
              rows={5}
              placeholder="Ingrese la modalidad de la maestria."
      ...
      {showAlert && (
        ...
        >
          Esta seguro que desea actualizar esta maestria?
        </SweetAlert>
      )}
      {showAlertValidation && (
        <SweetAlert
