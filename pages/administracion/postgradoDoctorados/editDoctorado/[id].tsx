...
function EditDoctorado({ id }: props) {
  const router = useRouter();
  const [showAlert, setShowAlert] = useState<boolean>(false);
  const [doctorado, setDoctorado] = useState<IDoctoradoData>();
  ...
  useEffect(() => {
    async function doFetch() {
      fetch(`${URL.baseUrl}/api/Carrera/Obtener/${id}`)
        .then((res) => res.json())
        .then((res) => {
          const doctoradoData = convertJSONDoctorado(res.response);
          console.log("Datos de la API:", doctoradoData); // Agregar este console.log
          setDoctorado(doctoradoData);
        })
        .catch((error) => {
  ...
  useEffect(() => {
    if (doctorado?.titulo) {
      setTitulo(doctorado!.titulo);
      setModalidad(doctorado!.modalidad);
      setImagen(doctorado!.imagen);
    }
  }, [doctorado]);
  ...
  return (
    <Layout>
      ...
      <div className="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
        <form id="miFormulario" onSubmit={handleSubmit}>
          <Label>
            ...
            <Input
              className="mt-1"
              placeholder="Ingresa el titulo del doctorado."
              value={titulo}
          ...
          <Label className="mt-3">
            ...
            <Textarea
              ...
              className="mt-1"
              rows={5}
              placeholder="Ingrese la modalidad del doctorado."
      ...
      {showAlert && (
        ...
        >
          Esta seguro que desea actualizar este doctorado?
        </SweetAlert>
      )}
      {showAlertValidation && (
        <SweetAlert
